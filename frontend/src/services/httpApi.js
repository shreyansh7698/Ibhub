/**
 * Real-backend implementation, used by `services/api.js` for every call.
 * Auth token is read from the auth service (sessionStorage) and sent as a
 * Bearer header; document bytes are never handled here beyond a multipart
 * POST and a fetch of a server-issued URL/stream.
 */
...
export const listMyApplications = () => req('/applications');
...
export async function getDocumentBlob(docId, applicationId) {
  // Same rule req() uses: admin pages carry the admin's token, everything
  // else carries the applicant's — this call just isn't routed through req().
  const token = window.location.pathname.startsWith('/admin') ? getAdminToken() : getUserToken();
...
export const getMyProfile = () => req('/auth/me');
